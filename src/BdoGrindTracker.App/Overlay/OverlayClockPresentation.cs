...
public static class OverlayClockPresentation
{
    ...
    public static OverlayClockView Create(OverlayWidget widget, DateTimeOffset now, TimeZoneInfo? localTimeZone = null)
    {
        ...
        if (widget.ShowRealTime || (!widget.ShowGameTime && !widget.ShowDayNightCountdown))
        {
            ...
            rows.Add(new("real", "Lokal", local.ToString(widget.ClockShowSeconds ? "HH:mm:ss" : "HH:mm", CultureInfo.InvariantCulture)));
        }
        if (widget.ShowGameTime)
            rows.Add(new("game", "BDO", state.GameTime.ToString(widget.ClockShowSeconds ? @"hh\:mm\:ss" : @"hh\:mm", CultureInfo.InvariantCulture)));
        if (widget.ShowDayNightCountdown)
        {
            // Round remaining duration up, so the display never reports zero
            // before the actual transition or loses almost a minute when compact.
            var unitTicks = widget.ClockShowSeconds ? TimeSpan.TicksPerSecond : TimeSpan.TicksPerMinute;
            ...
            rows.Add(new("countdown", state.IsDaytime ? "Nacht in" : "Tag in",
                rounded.ToString(widget.ClockShowSeconds ? @"hh\:mm\:ss" : @"hh\:mm", CultureInfo.InvariantCulture)));
        }

# Implementierungs- und Versionsgeschichte

Dieses Dokument bewahrt den vollständigen README-Stand vor der Aufräumung für
**1.4.1**. Es enthält die damalige Produktbeschreibung für **1.4.0**, ältere
Versionshinweise, Messwerte und technische Entscheidungsgründe. Aussagen wie
„aktuell“, „jetzt“ oder „nächste Version“ beziehen sich auf diesen historischen
Stand; Pfade und Bedienhinweise können durch spätere Änderungen abweichen.

Für Installation, aktuelle Bedienung und Entwicklung gilt die [README](../README.md).
Änderungen nach diesem Archivstand stehen in den
...
## Archivierter README-Stand

Lokaler, passiver Loot-Tracker für Black Desert, Version **1.4.0** (bisher BDO
...
Darstellung getrennt; es wird kein Webserver gestartet. Die Tracker-Oberfläche
bleibt lokal. Öffentliche Garmoth-Referenzen lädt ein separater Datenbrowser.

[Änderungen in 1.4.0](release-notes/1.4.0.md): OCR-Installation mit
Fortschrittsanzeige und erneuter Prüfung während des Wartens, weitere Grindspots
und mehrere individuell einstellbare Overlays.
...
[Verfahren, Messwerte und Prüfgrenzen](LIFETIME_LOOT_TRACKING.md).
...
[Garmoth-Analyse vom 10. September 2026](GARMOTH_REVERSE_ENGINEERING.md)
...
[Architektur, Voraussetzungen und UI-Prüfung](BLAZOR_HYBRID.md).
...
Die [gezielte Mengenprüfung](TRASH_QUANTITY_ANOMALIES.md) lernt die übliche
...
[Änderungen in 1.2.1](release-notes/1.2.1.md).

Der aktuelle Entwicklungsstand unterstützt alle **40 Spots aus den Referenz-Screenshots**, einschließlich aller zwölf bisherigen Edania-Zonen und ihrer konkreten Varianten.
Die Ergänzungen umfassen unter anderem Gavinya, Star's End, Sycraia, Elvia- und Dehkia-Spots.
Bei identischem Trashloot werden Floodlands-Gebiet, Ash-Forest-Stufe beziehungsweise
Winter Tree Fossil (280ap) vor dem Garmoth-Upload in der Live-Session ausgewählt.
[Spotliste, Itemdaten und Quellen](SCREENSHOT_SPOTS.md).
Zu den sechs Inner-Edania-Zonen kommen **Aetherion Castle, Nymphamaré Castle,
Orbita Castle, Tenebraum Castle, Zephyros Castle und Dark Energy Floodlands** hinzu.
Die Erweiterung umfasst automatische Erkennung am jeweiligen Trashloot,
englische und deutsche Itemnamen, Lootpools, Silberbewertung, Spotbilder und
Garmoth-Zuordnung aller konkreten Gebiete. Die Teilgebietsauswahl für Dark Energy
Floodlands ist jetzt in der Live-Session verfügbar. Trash-Maxima und die häufigen
Materialmengen folgen den aktuellen Nutzervorgaben; bisherige Trash-Minima bleiben
erhalten. Unbelegte seltene Dropmengen bleiben offen.
[Quellen und Abdeckung der Outer-Edania-Spots](OUTER_EDANIA_SOURCES.md).
...
Capture-Takt und Zählalgorithmus bleiben unverändert. [Details und Grenzen](OCR_RECOVERY.md).
...
[Lootpools](LOOT_POOLS.md). OCR, Matchingregeln und Zählung bleiben unverändert.

## Benutzung

### Ingame-Overlay

Unter **Overlay** kannst du mehrere unabhängige Fenster erstellen, benennen und
duplizieren. Pro Fenster lassen sich Metriken, Lootlisten, ein Silberverlauf,
Tracking-Buttons und eine Uhr mit Lokalzeit, BDO-Zeit und Tag-/Nacht-Countdown frei
anordnen. Position, Größe, Transparenz, Sichtbarkeit und Mausverhalten werden pro
Fenster automatisch gespeichert. Neue Fenster sind zunächst ausgeschaltet und
lassen sich vorab am Desktop testen; das bisherige Overlay wird übernommen.
[Bedienung und technische Grenzen](OVERLAY.md).
...
die Mengen-OCR. [Dropmengen und Korrekturen](DROP_QUANTITIES.md).
...
Grenzen: [Klassenerkennung](CLASS_DETECTION.md).
...
mit deinem Konto wurde nicht durchgeführt. [Details](GARMOTH_INTEGRATION.md).
...
über den Silberwerten; [Preisquellen und Bewertungsgrenzen](SILVER_VALUATION.md).

## Erkennung und Zählung

- Companion-Kalibrierung, Bildaufbereitung, Ziffernvorlagen, OCR, Textreparaturen
  und globaler Katalogmatcher bilden den ersten Erkennungsweg.
- Fehlende Mengen und nicht erkannte Zeilen erhalten in beiden Logs zusätzliche
  Leseversuche auf den Originalpixeln. Die Special-Zeile verwendet ebenfalls
  HDR-/Tone-Mapping-Aufbereitung, begrenzte Recovery und gezielte zweite OCR
  mit denselben Geometrie-, Namens-, Mengen- und Spotprüfungen. Ihr eigenes
  Recovery-Budget bleibt auch bei ausgelastetem normalen Log verfügbar.
  Pro Zeilenplatz entsteht höchstens eine abschließende Beobachtung.
...
- Unvollständige Namen können bereits erkannte Items stützen. Offene Rohlesungen
  ...
  [Konkrete Regeln und Grenzen](LIFETIME_LOOT_TRACKING.md).
- Das Special-Droplog verwendet dieselben korrigierbaren Namens-/Mengenevidenzen
  in einem eigenen Modell für genau eine Zeile. Eine dauerhaft sichtbare Meldung
  bleibt ein Drop; kurze OCR-Aussetzer erzeugen keinen weiteren. Ein bestätigter
  Itemwechsel oder eine beobachtete Abwesenheit von mindestens 1.550 ms trennt
  Meldungen. Die Belegungsprüfung für nachrückende normale Zeilen gilt nur dort,
  wo tatsächlich mehrere Feedpositionen vorhanden sind.
- Ein physischer Drop erscheint ausschließlich in einem der beiden Logs.
  Die getrennten Summen und Dropanzahlen werden deshalb addiert, auch bei
  gleichem Itemnamen. Wiederholungsprüfungen arbeiten innerhalb jeder Quelle.
  Historische Diagnose-Aufnahmen behalten ihre ursprünglichen Zähler und die
  damalige Vorrangregel für positive Normalsummen.
...
- Mengenkorrekturen beider Quellen ändern die Summen, zählen aber nicht als neue
...
unabhängig in Breite und Höhe verändern. Module, Texte und Icons behalten ihre
Position und Größe; Inhalte außerhalb der Fensterfläche werden nur abgeschnitten.
Die Hotkeys schalten alle Overlay-Fenster gemeinsam ein/aus beziehungsweise
zwischen Verschieben und Mausklicks ans Spiel um.
[Bedienung und Optionen](OVERLAY.md).
...
Testgrenzen stehen in [LIFETIME_LOOT_TRACKING.md](LIFETIME_LOOT_TRACKING.md).
...
[OCR_COMPARISON_20260911.md](OCR_COMPARISON_20260911.md) dokumentiert.
...
[Diagnosefelder und Vergleichsgrenzen](LIFETIME_LOOT_TRACKING.md).
...
Siehe [Sicherheitsgrenze](SAFETY.md) und [Architektur](ANALYSIS.md).
...
[Spielsprachen und Quellen](GAME_LANGUAGES.md). Das veröffentlichte Windows-x64-Paket enthält die
...
Companion-Version stehen im [Paritätsnachweis](COMPANION_0_7_4_PARITY.md).

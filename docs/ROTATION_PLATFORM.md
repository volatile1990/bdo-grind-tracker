...
A run picked up at a message several steps share is not guessed for good. `RotationAlignment` walks every candidate start (and, where the AFK end opens the next rotation, the previous rotation's end) through the messages that followed until the next pause, across cycle and rotation ends, and counts the required steps each would have to skip. A banner that is always read makes its absence evidence: a Magaia final phase without Elion's Tears is no first cycle, so two such cycles in a row can only be the second and third. Once one candidate needs fewer skips than every other, the run is replayed from its first message at that step (decision `placed`, earlier decisions are corrected); it is certain from its beginning and the Rotation Monitor draws it in full. While the position is still a guess, the guessed phase's timeout does not apply. Required steps skipped within what was observed are reported as `MissingSections` and drawn as tracking errors where they fell.
